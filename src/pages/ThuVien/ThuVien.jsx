...
const ThuVien = () => {
  ...
  const triggerDeleteItem = (id, type, name) => {
    setConfirmModal({
        ...
        id,
        itemType: type,
        message: `Bạn có chắc chắn muốn xóa ${type === 'tailieu' ? 'tài liệu' : 'câu hỏi'} "${name.substring(0, 50)}${name.length > 50 ? '...' : ''}" không?`,
    });
  };
        ...
        <section className="bg-white py-4 border-bottom shadow-sm">
          <div className="container">
              ...
              <div className="col-12 d-flex flex-wrap justify-content-between align-items-center gap-3">
                ...
                {(userRole === 'GiaoVien' || userRole === 'QuanTriVien') && (
                  <div className="d-flex gap-2">
                    <button className="btn btn-success px-4 py-2 shadow-sm fw-bold" onClick={() => navigate('/them-cau-hoi')}>
                      <i className="bi bi-plus-square-fill me-2"></i> Thêm câu hỏi
                    </button>
                    <button className="btn btn-primary px-4 py-2 shadow-sm fw-bold" onClick={() => navigate('/them-tai-lieu')}>
            ...
            <div className={`input-group mb-3 shadow-sm ${styles.filterControl}`}>
              ...
              <input type="text" className="form-control border-start-0 shadow-none" placeholder="Tìm kiếm nội dung tài liệu hoặc câu hỏi..." value={searchTerm} onChange={(e) => handleFilterChange(setSearchTerm, e.target.value)} />
            </div>
