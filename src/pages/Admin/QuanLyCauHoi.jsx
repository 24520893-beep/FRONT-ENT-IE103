...
const QuanLyCauHoi = () => {
    ...
    const fetchQuestions = useCallback(async () => {
        ...
        try {
            ...
            // Sử dụng fetchClient, giao phó việc lọc dữ liệu và phân trang cho Backend
    ...
    const handleUpdateStatus = async (id, newStatus) => {
        ...
        if (newStatus === 'Hoàn thiện') msg = "Xác nhận DUYỆT câu hỏi này?";
        if (newStatus === 'Đã từ chối') msg = "Xác nhận TỪ CHỐI câu hỏi này?";
        if (newStatus === 'Đang kiểm duyệt' && activeTab === 'Hoàn thiện') msg = "Xác nhận THU HỒI câu hỏi về trạng thái chờ duyệt?";
        if (newStatus === 'Đang kiểm duyệt' && activeTab === 'Đã từ chối') msg = "Xác nhận ĐƯA LẠI câu hỏi vào danh sách chờ duyệt?";
                                    ...
                                    <tr key={q._id}>
                                        ...
                                        <td className="pe-4 text-end">
                                            <div className="btn-group shadow-sm">
                                                <button className="btn btn-sm btn-light border" title="Xem chi tiết" onClick={() => navigate(`/thu-vien/cau-hoi/${q._id}`)}>
                                                    <i className="bi bi-eye text-primary"></i>
                                                </button>
                                                {activeTab === 'Đang kiểm duyệt' && (
                                                    <>
                                                        <button className="btn btn-sm btn-light border text-success fw-bold" onClick={() => handleUpdateStatus(q._id, 'Hoàn thiện')} title="Phê duyệt">
                                                            <i className="bi bi-check-lg me-1"></i> Duyệt
                                                        </button>
                                                        <button className="btn btn-sm btn-light border text-danger fw-bold" onClick={() => handleUpdateStatus(q._id, 'Đã từ chối')} title="Từ chối">
                                                            <i className="bi bi-x-lg me-1"></i> Từ chối
                                                        </button>
                                                    </>
                                                )}
                                                {activeTab === 'Hoàn thiện' && (
                                                    <button className="btn btn-sm btn-light border text-warning fw-bold" onClick={() => handleUpdateStatus(q._id, 'Đang kiểm duyệt')} title="Thu hồi">
                                                        <i className="bi bi-arrow-counterclockwise me-1"></i> Thu hồi
                                                    </button>
                                                )}
                                                {activeTab === 'Đã từ chối' && (
                                                    <button className="btn btn-sm btn-light border text-info fw-bold" onClick={() => handleUpdateStatus(q._id, 'Đang kiểm duyệt')} title="Khôi phục">
